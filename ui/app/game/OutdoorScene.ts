/**
 * 외부 전경 씬 (OutdoorScene) — 탑뷰 마을
 * 2×3 구조: 뒷줄 [집A][🏢HQ][집B] / 대로 / 앞줄 [집C][🌳공원][집D]
 * HQ 입구 클릭 시 OfficeScene 복귀
 * 에셋: Hoenn 타일셋에서 크롭한 건물 PNG 사용
 */
...
const BACK_BOTTOM = 240;
const ROAD_TOP = 260;
const ROAD_BOTTOM = 340;
const FRONT_TOP = 362;
const FRONT_BOTTOM = 528;
...
export default class OutdoorScene extends Phaser.Scene {
  ...
  preload() {
    ...
    bldgs.forEach(key => {
      if (!this.textures.exists(`bld_${key}`)) {
        this.load.image(`bld_${key}`, `/assets/buildings/${key}.png?${v}`);
      }
    });
  }
  ...
  private drawSkyAndGround() {
    ...
    if (this.isNight) {
      ...
      for (let i = 0; i < 25; i++) g.fillCircle((i * 113) % W, (i * 17) % (SKY_H - 10) + 4, 1);
    }
    ...
    for (let x = -10; x < W; x += 20) {
      const hh = 10 + ((x * 37) % 18);
      g.fillTriangle(x, SKY_H, x + 10, SKY_H - hh, x + 20, SKY_H);
    }
  }
  ...
  private placeHQ(x: number, y: number) {
    ...
    if (tex) tex.setFilter(Phaser.Textures.FilterMode.NEAREST);
    img.setScale(BUILDING_SCALE * 1.15);  // HQ 살짝 더 크게
    // 골드 글로우
    ...
    const zoneH = img.displayHeight * 0.35;
  ...
  private drawPark(col: number) {
    ...
    // 나무 4그루
    const trees = [[x - w / 2 + 28, yTop + 32], [x + w / 2 - 28, yTop + 32],
                   [x - w / 2 + 28, yBot - 32], [x + w / 2 - 28, yBot - 32]];
    trees.forEach(([tx, ty]) => {
      g.fillStyle(0x3a2818, 1); g.fillRect(tx - 2, ty + 10, 4, 10);
      g.fillStyle(this.isNight ? 0x0f2010 : 0x2a6a22, 1); g.fillCircle(tx, ty, 18);
      g.fillStyle(this.isNight ? 0x1a3018 : 0x4ab038, 0.85); g.fillCircle(tx - 5, ty - 5, 8);
    });
    // 벤치 2개
    [[x, yTop + 40], [x, yBot - 40]].forEach(([bx, by]) => {
      g.fillStyle(0x6a4828, 1);
      g.fillRect(bx - 18, by, 36, 5);
      g.fillRect(bx - 16, by + 5, 3, 7);
      g.fillRect(bx + 13, by + 5, 3, 7);
    });
    // 분수
    g.fillStyle(0x888898, 1); g.fillCircle(x, yTop + h / 2, 16);
    g.fillStyle(0x6ab8e8, 1); g.fillCircle(x, yTop + h / 2, 12);
    g.fillStyle(0xaadef8, 0.65); g.fillCircle(x - 3, yTop + h / 2 - 3, 5);
  ...
  private drawHQSign() {
    ...
    g.fillRoundedRect(x - 94, y - 22, 188, 28, 8);
    g.lineStyle(2, 0xf5c842, 1);
    g.strokeRoundedRect(x - 94, y - 22, 188, 28, 8);
    this.add.text(x, y - 8, "🏢 두근 컴퍼니", {
      fontSize: "15px", fontFamily: FONT, color: "#f5c842",
      fontStyle: "700", resolution: DPR * 4,
    }).setOrigin(0.5).setDepth(12);

    this.add.text(x, BACK_BOTTOM - 4, "▼ 입구 클릭", {
      fontSize: "10px", fontFamily: FONT, color: "#f5c842",
      fontStyle: "700", resolution: DPR * 4,
  ...
  private drawMainRoad() {
    ...
    for (let x = 10; x < W; x += 52) g.fillRect(x, midY, 32, 4);
    // 횡단보도 (HQ 정면)
    const hqX = this.colCenters[HQ_COL];
    g.fillStyle(0xffffff, this.isNight ? 0.55 : 0.85);
    for (let i = 0; i < 7; i++) g.fillRect(hqX - 32, ROAD_TOP + 4 + i * 10, 64, 5);
    // 인도 (대로 위·아래)
  ...
  private drawBottomSidewalk() {
    ...
    g.lineStyle(1, 0x3a3a48, 0.4);
    for (let xx = 0; xx < W; xx += 26) g.lineBetween(xx, FRONT_BOTTOM, xx, SIDEWALK_BOTTOM);
  }
  ...
  private drawStreetFurniture() {
    ...
    for (let i = 0; i < COLS - 1; i++) {
      ...
      [BACK_BOTTOM + 12, ROAD_BOTTOM + 16].forEach(ty => {
        ...
        g.fillStyle(this.isNight ? 0x0f2010 : 0x2a6a22, 1); g.fillCircle(alleyX, ty - 2, 8);
  ...
  private initWeather() {
    ...
    const isRain = (wc >= 51 && wc <= 82) || wc >= 95;
    const isSnow = wc >= 71 && wc <= 77;
    if (isRain) for (let i = 0; i < 40; i++) this.rainDrops.push({
      x: Math.random() * W, y: Math.random() * H, speed: 2 + Math.random() * 1.5, len: 5 + Math.random() * 4 });
    if (isSnow) for (let i = 0; i < 35; i++) this.snowFlakes.push({
