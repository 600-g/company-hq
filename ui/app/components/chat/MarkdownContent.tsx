    ...
    <div className={`prose prose-sm dark:prose-invert max-w-none break-words ${className ?? ""}`}>
      <ReactMarkdown
        components={{
          ...
          pre: ({ children }) => <div className="my-1.5 overflow-hidden rounded-md">{children}</div>,
          blockquote: ({ children }) => (
            <blockquote className="border-l-2 border-gray-600 pl-3 my-1.5 italic text-gray-400">
              {children}
            </blockquote>
          ),
