...
def get_zone_id(zone_name: str | None = None) -> str | None:
    ...
    if not _get_token():
        logger.warning("[cf_dns] CF_TOKEN 미설정")
        return None
    try:
        r = requests.get(f"{CF_API}/zones?name={zone}", headers=_headers(), timeout=8)
        r.raise_for_status()
...
def find_record(prefix: str, zone_name: str | None = None) -> dict | None:
...
def add_subdomain(
    ...
    반환: {ok, record_id?, full_name?, url?, error?}
    """
    if not _get_token():
        return {"ok": False, "error": "CF_TOKEN 미설정 (.env 에 추가하세요)"}
    ...
    existing = find_record(prefix, zone)
    if existing:
        return {
            "ok": True,
            "skipped": "이미 존재",
    ...
    try:
        ...
        logger.info("[cf_dns] %s CNAME → %s 등록 완료", full_name, target)
        return {
            "ok": True,
...
def delete_subdomain(prefix: str, zone_name: str | None = None) -> dict[str, Any]:
