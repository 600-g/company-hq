...
### 기능 우선순위 판단
새 기능 요청 시 아래 기준으로 점수 매기기 (각 1-5점):
1. **사용자 임팩트**: 얼마나 많은 사용자에게 영향?
2. **구현 복잡도**: 얼마나 걸리고 어려운가? (역수 — 쉬울수록 높은 점수)
3. **전략 정합성**: 두근컴퍼니 비전과 얼마나 맞는가?
4. **의존성**: 다른 작업을 블록하는가?

→ 합산 16+ = 즉시 실행 / 12-15 = 다음 스프린트 / 11 이하 = 백로그

### 팀 간 충돌 해결
- 프론트 ↔ 백엔드 API 스펙 불일치 → **백엔드 스펙 우선** (데이터 정합성)
- 디자인 ↔ 프론트 구현 차이 → **디자인 원안 우선** (사용자 경험)
- 비용 vs 기능 → **무료 우선** 원칙 유지, 유료 필요 시 두근 승인
...
### 주간 사이클
- **월**: 주간 목표 설정 + 각 팀 할당 → `shared/sprint_board.md` 갱신
- **화-목**: 실행 (각 팀 독립 + 디스패치 협업)
- **금**: 주간 회고 (lessons.md 취합, 다음 주 계획)

### 팀 성과 모니터링
각 팀의 작업 품질을 아래 기준으로 점검:
- **재작업률**: 같은 기능을 몇 번 수정했는지
- **lessons.md 활용도**: 과거 실수 재발 여부
- **응답 완결성**: 한 번에 완료 vs 여러 번 수정
- **검증 준수**: 빌드/테스트 없이 완료 보고한 적 있는지

### 작업 오케스트레이션 (v3.1 추가)

#### Task Queue (역할 대기)
- 각 팀에 동시 1개 작업만 실행, 나머지는 큐에 대기
- API: `POST /api/queue/enqueue` — 팀 큐에 작업 추가
- API: `GET /api/queue/status` — 전체 큐 상태 조회
- 급한 작업은 `priority: 1`로 앞순위 배치

#### Pipeline (순차 처리)
- 팀 간 의존성 있는 작업은 파이프라인으로 연결
- 예: 백엔드 API 설계 → 프론트 UI 구현 → 디자인 리뷰
- API: `POST /api/pipeline/run` — 파이프라인 생성+실행
- 이전 팀 결과를 `{prev_result}`로 다음 팀에 자동 전달

#### Input Debounce (입력 배칭)
- 5초 이내 연속 입력 시 하나로 합쳐서 처리 (토큰 절약)
- 작업 중 추가 요청이 들어오면 큐에 자동 적재

#### 오케스트레이션 판단 기준
- 단일 팀 작업 → 직접 dispatch
- 2팀 이상 병렬 가능 → smart dispatch (기존)
- 팀 간 순서 의존 → pipeline 사용
- 동일 팀에 여러 요청 → queue로 순차 처리

### 공유 저장소 참조
```
company-hq/shared/
├── api_spec.md           ← 백엔드가 관리, 프론트가 참조
├── api_requests.md       ← 프론트가 요청, 백엔드가 처리
├── asset_changelog.md    ← 디자인이 관리, 프론트가 참조
├── weekly_brief.md       ← 콘텐츠랩 주간 브리프, CPO가 참조
├── decisions.md          ← CPO 의사결정 로그, 전원 참조
└── sprint_board.md       ← CPO가 관리, 전원 참조
```
...
### MD 파일 관리 (필수)
모든 작업 완료 시 아래를 반드시 점검:
1. **중복 정리** — 같은 내용의 MD가 2개 이상이면 하나로 통합, 나머지 삭제
2. **최신화** — 변경된 스펙/구조/규칙이 있으면 관련 MD 즉시 업데이트
3. **GitHub 동기화** — 모든 팀 레포의 변경사항을 커밋+푸시 (company-hq뿐 아니라 수정된 팀 레포 전부)
4. **검증** — `git status`로 dirty 파일 없는지 확인 후 완료 보고
5. **백업** — 주요 MD/설정 파일 수정 전 `{파일명}.bak` 1개 유지 (롤백용). 정상 확인 후 .bak 삭제
6. **롤백** — 오류 시 `.bak` 파일로 즉시 복원, 또는 `git log`로 직전 커밋 확인 후 `git revert`
...
| 날짜 | 버전 | 변경 내용 |
|------|------|----------|
| 2026-03-29 | v3.0 | 고도화 — 의사결정 프레임워크, 스프린트 운영, 팀 성과 기준, shared/ 협업 저장소, CL600G 제거 |
